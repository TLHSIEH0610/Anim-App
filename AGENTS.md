...
- Stripe secret/publishable keys must be present in `infra/.env`; publishable key also lives in `frontend/.env`.
 - Free‑trial verification ($0): `POST /billing/setup-intent-free-trial` → show SetupIntent PaymentSheet → `POST /billing/free-trial-verify-complete` → book creation with `apply_free_trial=true`.

## 10. Client Attestation & Audit (2025‑11)
- Client headers (best‑effort):
...
- Persistence & visibility:
  ...
  - Admin Portal: Users page shows attestation snippet; top menu links to “Audit” page to browse logs.
  - Card verification: `users.card_verified_at` is set when a Stripe card payment is confirmed (paid book) or after the $0 free‑trial SetupIntent completes. Shown on the Users page.
  - Last login: `users.last_login_at` updated on successful email/password or Google login; displayed in Admin → Users.
