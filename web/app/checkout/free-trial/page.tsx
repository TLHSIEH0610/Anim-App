...
function FreeTrialInner({ template_slug }: { template_slug: string }) {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (!r.ok) throw new Error(await r.text())
      // Kick off book creation with free trial
      window.location.href = `/create?template_slug=${encodeURIComponent(template_slug)}&apply_free_trial=true`
    } catch (e: any) {
      setError(e.message)
    } finally {
      setLoading(false)
...
export default function FreeTrialPage({ searchParams }: { searchParams: { template_slug?: string } }) {
  const slug = searchParams.template_slug || 'base'
  return (
    <StripeProvider>
      <FreeTrialInner template_slug={slug} />
    </StripeProvider>
  )
}
