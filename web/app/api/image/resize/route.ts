...
export async function GET(req: NextRequest) {
  ...
  const w = url.searchParams.get('w') || '360'
  const h = url.searchParams.get('h') || ''
  const v = url.searchParams.get('v') || ''
  if (!path) return NextResponse.json({ error: 'Missing path' }, { status: 400 })
  const jwt = req.cookies.get('auth_token')?.value
  if (!jwt) return NextResponse.json({ error: 'Not authenticated' }, { status: 401 })
  const qs = new URLSearchParams({ path, w, ...(h ? { h } : {}), token: jwt, ...(v ? { v } : {}) })
  const r = await fetch(`${API_BASE}/books/media/resize-public?${qs.toString()}`, { cache: 'no-store' })
